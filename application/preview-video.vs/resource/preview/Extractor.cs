        ...
        protected override void _Execute(atom.Trace context, int level, string url, string file)
        {
            var a_Context = ImageMetadataReader.ReadMetadata(file);
            {
                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.HEADER, level, "[[[Info]]]");
                {
                    ...
                    context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 1, "[[[Media Types]]]", "Audio, Video");
                    context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 1, "[[[Raw Format]]]", __GetString(a_Context, "Expected File Name Extension").ToUpper());
                    context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 1, "[[[Tag Types]]]", __GetString(a_Context, "Detected File Type Name"));
                }
            }
            {
            ...
            {
                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.FOOTER, level, "[[[Size]]]: " + __GetInteger(a_Context, "Width").ToString() + " x " + __GetInteger(a_Context, "Height").ToString());
                {
                    context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.FOLDER, level + 1, "[[[Codecs]]]");
                    {
                        context.
                            SetComment("[[[Audio]]]", "[[[Media Type]]]").
                            Send(NAME.SOURCE.PREVIEW, NAME.TYPE.FILE, level + 2, __GetString(a_Context, "Expected File Name Extension").ToUpper() + " Audio (" + __GetString(a_Context, "Major Brand") + ")");
                        {
                            context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 3, "[[[Header]]]");
                            {
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Channels]]]", __GetChannels(a_Context).ToString());
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Duration]]]", __GetDuration(a_Context));
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Media Types]]]", "[[[Audio]]]");
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Preferred Rate]]]", __GetInteger(a_Context, "Preferred Rate").ToString());
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Preferred Volume]]]", __GetInteger(a_Context, "Preferred Volume").ToString());
                            }
                        }
                    }
                    {
                        context.
                            ...
                            Send(NAME.SOURCE.PREVIEW, NAME.TYPE.FILE, level + 2, __GetString(a_Context, "Expected File Name Extension").ToUpper() + " Video (" + __GetString(a_Context, "Major Brand") + ")");
                        {
                            context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 3, "[[[Header]]]");
                            {
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Duration]]]", __GetDuration(a_Context));
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Media Types]]]", "[[[Video]]]");
                            }
                        }
                        {
                            context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 3, "[[[Size]]]");
                            {
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Width]]]", __GetInteger(a_Context, "Width").ToString());
                                context.Send(NAME.SOURCE.PREVIEW, NAME.TYPE.PARAMETER, level + 4, "[[[Height]]]", __GetInteger(a_Context, "Height").ToString());
                            }
                        }
                    }
                }
